...
def addtag(update, context):
    ...
    if match_user in tagall_list:
        ...
        return
    message.reply_text(
        "{} Hei hi pawm rawh, nangmah leh nangmah {}'s tag list lama i in dah duh chuan! Emaw **ka pawm lo** tih khu hmet rawh.".format(
            mention_html(member.user.id, member.user.first_name), chat.title
        ),
            ...
            [
                [
                    InlineKeyboardButton(
                        text="ka pawm e", callback_data=f"tagall_accept={user_id}"
                    ),
                    InlineKeyboardButton(
                        text="ka pawm lo", callback_data=f"tagall_dicline={user_id}"
                    ),
                ]
            ]
        ),
